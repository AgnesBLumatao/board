<table width="100%">
	<tr>
		...
		<td align="right">
		<h5>WELCOME <?php eh($username) ?> <a class="btn btn-small btn-primary" href="<?php eh(url('thread/index')) ?>">LOG OUT</a></h5>
		</td>
	</tr>
...
for($x=0;$x<$totalComment;$x++):?>
	<div class="comment">
		...
		<div class="meta">
		<?php eh($x+1) ?>:<?php eh($comments[$x]['created']) ?>
		</div>
...
<tr>
	...
	if($start!=1):?>
		<td>
		<a href="<?php eh(url('thread/view', array('page'=>$start-10,'thread_id' => $thread->id, 'username'=>$username,'user_id'=>$user_id))) ?>">
		&larr; previous
		</a>
		</td>
	<?php
